...
if(Meteor.isServer){
  import { AllergyIntolerances, CarePlans, CareTeams, CodeSystems, Conditions, Compositions, Claims, DiagnosticReports, Encounters, Endpoints, ExplanationOfBenefits, Immunizations, Lists, Measures, MeasureReports, Medications, MedicationOrders, MedicationStatements, Observations, Organizations, Patients, Procedures, Provenances, RiskAssessments, SearchParameters, ServiceRequests, StructureDefinitions, Tasks,  ValueSets } from 'meteor/clinical:hl7-fhir-data-infrastructure';
...
MedicalRecordImporter = {
  ...
  importNdjson: function(dataContent){
    console.log('MedicalRecordImporter.importNdjson', dataContent);

    if(Array.isArray(dataContent)){
      dataContent.forEach(function(record){

        if(get(record, 'resourceType')){
          console.log('Found a ' + get(record, 'resourceType'));
          ...
          if(Collections[MedicalRecordImporter.pluralizeResourceName(get(record, 'resourceType'))]){
            if(!Collections[MedicalRecordImporter.pluralizeResourceName(get(record, 'resourceType'))]._collection.findOne({_id: newRecord._id})){                  
              console.log('Couldnt find record; attempting to insert.')
              Collections[MedicalRecordImporter.pluralizeResourceName(get(record, 'resourceType'))]._collection.insert(newRecord, {validate: false, filter: false}, function(error){
                if(error) {
  ...
  importBundle: function(dataContent, proxyUrl){    
    ...
    if(typeof dataContent === "string"){
      let stringParts = dataContent.split('\n');
      if(Array.isArray(stringParts)){
        if(stringParts.length > 1){
          // this is for NDJSON
          // parsedResults = JSON.parse(stringParts[0]);
          ...
          if(get(entry, 'resource.resourceType')){
            ...
            if(Collections[self.pluralizeResourceName(get(entry, 'resource.resourceType'))]){
              ...
              if(Meteor.isClient && subscriptionActivated){
                ...
                console.log('Cursur appears to be offline.')
    ...
    // if it's not an array...
      ...
      if(Meteor.isClient){
        ...
        if(self.pluralizeResourceName(get(parsedResults, 'resource.resourceType'))){
          ...
          console.log('MedicalRecordImporter.pluralizedCollectionName', pluralizedCollectionName)
