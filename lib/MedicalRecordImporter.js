...
if(Meteor.isServer){
  import { AllergyIntolerances, Bundles, CarePlans, CareTeams, CodeSystems, Conditions, Compositions, Claims, DiagnosticReports, DocumentManifest, DocumentReference, Devices, Encounters, Endpoints, ExplanationOfBenefits, Immunizations, Lists, Measures, MeasureReports, Medications, MedicationOrders, MedicationStatements, Observations, Organizations, Patients, Procedures, Provenances, RiskAssessments, SearchParameters, ServiceRequests, StructureDefinitions, Tasks,  ValueSets } from 'meteor/clinical:hl7-fhir-data-infrastructure';
  ...
  Collections.DocumentManifest = DocumentManifest;
  Collections.DocumentReference = DocumentReference;
...
MedicalRecordImporter = {
  ...
  importNdjson: function(dataContent){
    console.log('--------------------------------------------------------')
        ...
        if(get(parsedRecord, 'resourceType')){
          ...
          if(Collections[MedicalRecordImporter.pluralizeResourceName(get(parsedRecord, 'resourceType'))]){
            if(!Collections[MedicalRecordImporter.pluralizeResourceName(get(parsedRecord, 'resourceType'))]._collection.findOne({_id: newRecord._id})){                  
              ...
              Collections[MedicalRecordImporter.pluralizeResourceName(get(parsedRecord, 'resourceType'))]._collection.insert(newRecord, {validate: false, filter: false}, function(error){
                if(error) {
  ...
  importBundle: function(dataContent, proxyUrl){    
    console.log('----------------------------------------------------');
    ...
    console.log('Parsed results typeof:  ', typeof parsedResults);
    console.log("get(parsedResults, 'resourceType')", get(parsedResults, 'resourceType'));
        ...
        parsedResults.entry.forEach(function(entry){          
          if(get(entry, 'resource.resourceType')){
            ...
            if(get(entry.resource, 'resourceType') === "Patient"){
              console.log(FhirUtilities.assembleName(get(entry.resource, 'name[0]')))
              set(newRecord, 'name[0].text', FhirUtilities.assembleName(get(entry.resource, 'name[0]')));
            }
            ...
            if(Collections[self.pluralizeResourceName(get(entry, 'resource.resourceType'))]){
              ...
              if(Meteor.isClient && subscriptionActivated){
                ...
                console.log('Cursor appears to be inactive.')
    ...
    // if it's not an array...
      ...
      if(get(parsedResults, 'resourceType')){
        console.log('Found a ' + get(parsedResults, 'resourceType'));
        ...
        if(Meteor.isClient){
          ...
          if(Collections[pluralizedCollectionName]){          
            if(!Collections[pluralizedCollectionName].findOne({id: parsedResults.id})){
              ...
              console.log("Couldnt find any records in the " + pluralizedCollectionName + ' collection.')
            }
          } else {
            console.log("Couldnt find the " + pluralizedCollectionName + ' collection.  Is it imported?')
          }
  
        } else {
          console.log('Not running on Client.  Skipping.')
        }  
      }
    }    
    return true
